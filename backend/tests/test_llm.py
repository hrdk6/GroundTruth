...
from tests.conftest import FakeAnthropic, FakeUsage
...
def test_temperature_dropped_for_models_that_reject_it(llm_client: LLMClient) -> None:
    fake = FakeAnthropic()
    llm_client._client = fake
    llm_client.complete("hi", model="claude-sonnet-5", temperature=0.0)
    assert "temperature" not in fake.calls[0]


def test_temperature_passed_through_when_supported(llm_client: LLMClient) -> None:
    fake = FakeAnthropic()
    llm_client._client = fake
    llm_client.complete("hi", model="claude-haiku-4-5", temperature=0.0)
    assert fake.calls[0]["temperature"] == 0.0


# --- caching --------------------------------------------------------------
def test_identical_calls_hit_the_cache(llm_client: LLMClient) -> None:
    fake = FakeAnthropic(text="cached answer")
    llm_client._client = fake
...
def test_cache_hit_costs_nothing_but_remembers_list_price(llm_client: LLMClient) -> None:
    llm_client._client = FakeAnthropic(usage=FakeUsage(input_tokens=1000, output_tokens=500))
...
def test_tracker_accumulates_across_calls(llm_client: LLMClient) -> None:
    llm_client._client = FakeAnthropic(usage=FakeUsage(input_tokens=100, output_tokens=50))
